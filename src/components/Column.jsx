  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, width: 285, flexShrink: 0, willChange: "transform" }}>
      ...
      <div onContextMenu={e => { e.preventDefault(); setContextMenu({ x: e.clientX, y: e.clientY }); }}
        style={{ background: theme.surface, border: `1px solid ${isDragging ? theme.accent + "60" : isLocked ? theme.accent + "30" : theme.border}`, borderRadius: theme.r2, overflow: "hidden", opacity: isDragging ? 0 : 1 }}>
        {/* HEADER */}
        <div style={{ padding: "11px 13px 9px", display: "flex", alignItems: "center", gap: 7, borderBottom: isCollapsed ? "none" : `1px solid ${theme.border}` }}>
          ...
          {editingTitle ? (
            <input
              ...
              style={{ flex: 1, background: "transparent", border: "none", borderBottom: `1px solid ${theme.accent}`, color: theme.text, fontFamily: theme.font || "Syne", fontSize: 13, fontWeight: 700, outline: "none", minWidth: 0 }}
            />
          ) : (
            <div
              onDoubleClick={() => !isLocked && setEditingTitle(true)}
              title={isLocked ? undefined : "Double-click to rename"}
              style={{ fontSize: 13, fontWeight: 700, flex: 1, color: theme.text, cursor: isLocked ? "default" : "text" }}
            >
